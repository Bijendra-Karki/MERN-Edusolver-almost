...
// NOTE: We don't need axios or isAuthenticated here, as all verification/enrollment is done 
// by the backend before we land on this page.
...
const PaymentSucces = () => {
   ...
   useEffect(() => {
        // 1. Existing logic to check URL params...
        ...
        if (subjectId && paymentId && refId) {
            ...
            // 🛑 CRITICAL FIX: Clear temporary session data
            sessionStorage.removeItem('currentSubjectPaymentDetails');
          

            // ... existing success status setting ...
        }   
        else {
            // Failure: The necessary success data is missing, likely due to an error 
            // during the verification process on the backend.
            setStatus('Verification Failed or Data Missing.');
            // Redirect to a failure page after a delay
            setTimeout(() => {
                    ...
                    <>
                        ...
                        <div className="text-left bg-green-50 p-4 rounded-lg border border-green-200">
                            <p className="text-sm text-green-800">
                                <strong>Course ID:</strong> {details.subjectId}
                            </p>
                            <p className="text-sm text-green-800">
                                <strong>Transaction ID (eSewa):</strong> {details.refId}
                            </p>
                            <p className="text-sm text-green-800">
                                <strong>Payment Record ID:</strong> {details.paymentId}
                            </p>
                        </div>
                        <button
